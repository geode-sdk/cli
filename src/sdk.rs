...
fn download_url(url: String, file_name: &PathBuf) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
...
fn switch_to_tag(config: &mut Config, repo: &Repository) {
	...
	if config.sdk_nightly {
		...
		repo.checkout_head(None).nice_unwrap("Unable to switch to latest commit");
		info!("Switched to latest commit");
		return;
	}

	let mut latest_version: Option<Version> = None;
	for tag in repo.tag_names(None).nice_unwrap("Unable to get SDK tags").iter() {
		if let Some(tag) = tag {
			if let Ok(version) = parse_version(tag) {
				if latest_version.as_ref().is_none() {
					latest_version = Some(version);
				} else if version > latest_version.as_ref().unwrap().to_owned() {
					latest_version = Some(version);
				}
			}
		}
...
fn install_binaries(config: &mut Config) {
	...
	zip.extract(target_dir).nice_unwrap("Unable to unzip downloaded binaries");
